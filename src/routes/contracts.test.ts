...
describe('agent contract alignment', () => {
  ...
  it('keeps the public docs pages aligned with the live wire contract', () => {
    ...
    expect(apiDocs).toContain('/api/check/batch')
    expect(apiDocs).toContain('/api/check/manifest')
    ...
    expect(apiDocs).toContain('policyVerdict')
    expect(apiDocs).toContain('X-IsItAlive-Client')
    ...
    expect(landing).toContain('Give your agents, pipelines, and READMEs a liveness check')
    ...
    expect(openApiSpec.paths).toHaveProperty('/api/check/batch')
    expect(openApiSpec.paths).toHaveProperty('/api/check/manifest')
    expect(openApiSpec.components.schemas.AuditRequest.properties.format.enum).toContain('package-lock.json')
    expect(openApiSpec.components.schemas.AuditDep.properties).toHaveProperty('identity')
    expect(openApiSpec.components.schemas.AuditDep.properties).toHaveProperty('state')
    expect(openApiSpec.components.schemas.AuditDep.properties).toHaveProperty('dataFreshness')
    expect(JSON.stringify(openApiSpec.paths)).toContain('X-IsItAlive-Client')
    ...
    expect(llmsTxt).toContain('/api/check/batch')
    expect(llmsTxt).toContain('package-lock.json')
    expect(llmsTxt).toContain('X-IsItAlive-Client')
    ...
    expect(aiPluginManifest.description_for_model).toContain('checkBatch')
    ...
    expect(agentsMd).toContain('/api/check/batch')
    expect(agentsMd).toContain('package-lock.json')
    expect(agentsMd).toContain('X-IsItAlive-Client')
