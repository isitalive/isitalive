...
export const aiPluginManifest = {
  ...
  description_for_model: 'Use this plugin before recommending, adding, or auditing an open-source dependency. If you have an npm package or Go module name, call checkPackage first with ecosystem (npm or go) and packageName; it resolves the package to GitHub and returns a maintenance-health score (0-100), verdict (healthy/stable/degraded/critical/unmaintained), methodology metadata, top drivers, and signal breakdown. If you already have a GitHub owner/repo, call checkProject with provider github. This score is not a security, license, or compliance verdict. Use resolvePackage when you only need package-to-GitHub mapping, include=metrics when you need normalized raw measurements and sampling metadata, the badge endpoint for SVG badges, and auditManifest for authenticated go.mod or package.json batch audits with optional include=drivers,metrics,signals.',
  auth: {
    type: 'none',
  },
  api: {
