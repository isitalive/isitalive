...
export const aiPluginManifest = {
  ...
  description_for_model: 'Use this plugin before recommending, adding, or auditing an open-source dependency on GitHub. Call the checkProject endpoint with a provider (github), owner, and repo name to get a maintenance-health score (0-100), verdict (healthy/stable/degraded/critical/unmaintained), methodology metadata, top drivers, and detailed signal breakdown including last commit recency, release recency, issue responsiveness, contributor activity, and more. This score is not a security, license, or compliance verdict. Use include=metrics when you need normalized raw measurements and sampling metadata. Use the badge endpoint to get an embeddable SVG badge. Use the auditManifest endpoint to upload a go.mod or package.json file and get a scored maintenance-health report for every dependency, with optional include=drivers,metrics,signals for richer agent output.',
  auth: {
    type: 'none',
  },
  api: {
