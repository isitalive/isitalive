...
const rawProjectDataArb: fc.Arbitrary<RawProjectData> = fc.record({
  ...
  issueSampleSize: fc.nat({ max: 50 }),
  issueSampleLimit: fc.constant(50),
  issueSamplingStrategy: fc.constant('median of the 50 most recently updated open issues'),
  prResponsivenessMedianDays: fc.option(fc.nat({ max: 3650 }), { nil: null }),
  prSampleSize: fc.nat({ max: 20 }),
  prSampleLimit: fc.constant(20),
  prSamplingStrategy: fc.constant('median of the 20 most recently updated open pull requests'),
  ...
  contributorCommitSampleSize: fc.nat({ max: 100 }),
  contributorWindowDays: fc.constant(90),
  ...
  ciWorkflowRunSampleSize: fc.nat({ max: 10 }),
  ciSamplingWindowDays: fc.constant(30),
  ciDataSource: fc.constantFrom('actions-runs', 'workflow-directory-only', 'actions-runs-unavailable', 'none'),
})
